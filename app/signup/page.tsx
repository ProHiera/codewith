...
export default function SignUpPage() {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user) {
        // Trigger가 자동으로 users 테이블에 레코드 생성
      ...
      if (error.message) {
        if (error.message.includes('For security purposes')) {
          ...
          errorMessage = '회원가입 권한이 없습니다. 관리자에게 문의해주세요.';
    ...
    try {
      ...
      if (error) {
        if (error.message.includes('not enabled')) {
          setError(`${provider.toUpperCase()} 로그인이 아직 설정되지 않았습니다. OAUTH_SETUP.md 파일을 참고하여 Supabase Dashboard에서 ${provider} 제공자를 활성화해주세요.`);
        } else {
          throw error;
        }
      }
        ...
        <form onSubmit={handleSignUp} className="mt-8 space-y-6">
          ...
          <div className="space-y-3">
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-3 py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-gray-900 bg-[#FEE500] hover:bg-[#FDD835]"
            >
              <svg className="w-5 h-5" viewBox="0 0 24 24">
